...
function format() {
  ...
  try {
    ...
    let formatted = ''
    let indent = 0
    const html = input.value.replace(/>\s*</g, '><').trim()
...
<template>
  <ToolLayout tool-id="html-formatter" title="HTML Formatter" description="Format and indent HTML">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <BaseCard variant="bordered" :padding="false">
        ...
        <textarea v-model="input" class="w-full h-96 p-4 font-mono text-sm bg-transparent border-0 resize-none focus:outline-none" placeholder="Paste HTML here..." spellcheck="false" />
      </BaseCard>
      <BaseCard variant="bordered" :padding="false">
